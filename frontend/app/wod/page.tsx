        ...
        <div className={s.heroInner}>
          <div className={s.heroLeft}>
            <div className={s.heroEyebrow}>
              <svg className={s.fireIcon} width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
          ...
          <div className={s.heroRight}>
            <Link href="/wod/timer" className={s.timerLink}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
              </svg>
              WOD 타이머
            </Link>
            <Link href="/wod/leaderboard" className={s.heroNavLink}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/>
              </svg>
              전체 리더보드
            </Link>
            <Link href="/wod/records" className={s.heroNavLink}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <div className={s.content}>
        <div className={s.contentGrid}>
            ...
            {isLoading ? (
              ...
              <div className={s.wodCard}>
                <div className={s.wodCardHeader}>
                  <div className={s.wodCardMeta}>
                    <span className={`badge ${WOD_TYPE_BADGE[todayWod.type] || "badge-default"}`}>
                      {WOD_TYPE_LABELS[todayWod.type] || todayWod.type}
                    </span>
                    {todayWod.scoreType && (
                      <span className={s.scoreChip}>{todayWod.scoreType}</span>
                    )}
                  </div>
                  <div className={s.wodCardActions}>
          ...
          <div className={s.sideCol}>
            ...
            {todayWod && leaderboard && leaderboard.length > 0 && (
              <div className={s.sideCard}>
                <div className={s.sideCardHeader}>
                  <p className={s.sideCardTitle}>오늘의 리더보드</p>
                  <Link href="/wod/leaderboard" className={s.sideCardMore}>전체 →</Link>
                </div>
            ...
            {todayWod && boxRanking && boxRanking.length > 0 && (
              <div className={s.sideCard}>
                <div className={s.sideCardHeader}>
                  <p className={s.sideCardTitle}>박스 랭킹</p>
                  <Link href="/wod/box-ranking" className={s.sideCardMore}>더보기 →</Link>
                </div>
        ...
        <div className={s.historySection}>
          <div className={s.viewToggle}>
            ...
            <Link href="/wod/history" className={s.historyMoreLink}>
              전체 히스토리 →
            </Link>
            ...
            <div>
              <div className={s.historyList}>
                {historyData?.content?.map((wod: Wod) => {
                  const isSelected = selectedWod?.id === wod.id;
                  return (
                    <div
                      key={wod.id}
                      className={`${s.historyItem} ${isSelected ? s.historyItemSelected : ""}`}
                      onClick={() => setSelectedWod(isSelected ? null : wod)}
                    >
                      ...
                      <div className={s.historyBody}>
                        <div className={s.historyBodyTop}>
                      ...
                      <svg
                        className={`${s.historyChevron} ${isSelected ? s.historyChevronOpen : ""}`}
                ...
                <div className={s.historyDetail}>
                  <div className={s.historyDetailHeader}>
                    <div>
                      <div className={s.historyDetailMeta}>
                        ...
                        <span className={s.historyDetailDate}>
                          {dayjs(selectedWod.wodDate).format("YYYY년 MM월 DD일 ddd")}
                        </span>
                      </div>
                      <p className={s.historyDetailTitle}>{selectedWod.title}</p>
                    </div>
                  ...
                  {selectedWod.imageUrl && (
                    <div style={{ position: "relative", width: "100%", height: 280, marginTop: 16 }}>
                      <Image src={selectedWod.imageUrl} alt="WOD" fill style={{ objectFit: "cover" }} />
                    </div>
                  )}
                </div>
              )}
